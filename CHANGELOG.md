...
### Fixed
- **ICON Import Errors**: Removed non-existent `ICON_ALERT_CIRCLE` and `ICON_ACCOUNT` from `binary_sensor.py`
  - These icon constants don't exist in ESPHome 2025.10.3
  - Platform now imports correctly without import errors

### Added
- **Velocity Sensor Support**: Added velocity coordinate type to sensor platform
  - Extended `CoordinateType` enum with `VELOCITY = 3`
  - Added `register_velocity_sensor()` registration in `sensor.py`
  - Velocity sensors can now be properly configured in YAML

### Changed
- Binary sensor platform now uses only device classes without custom icons
- Sensor coordinate types now support: x, y, z, velocity
